...
export function LoginPage() {
  ...
  return <main className="auth-page">
    <section className="auth-panel"><div className="auth-brand"><BrandMark /><strong>BERT CRM</strong></div><div className="auth-copy"><span className="eyebrow">Внутрішній простір команди</span><h1>Робота, рішення й знання — в одному місці.</h1></div><img src="/assets/auth/editorial-workspace.png" alt="Світлий сучасний робочий простір BERT" width="720" height="480" /></section>
    <section className="login-card"><div><span className="eyebrow">З поверненням</span><h2>Увійти до BERT CRM</h2></div>
...
export function RestrictedAccessPage({ mode }: { mode: 'first-login' | 'password-changed' | 'help' | 'setup' | 'challenge' }) {
  ...
  if (mode === 'help') return <AccessCard variant="help" icon={<KeyRound />} title="Допомога з доступом" text="У BERT CRM немає відновлення через email. Зверніться до уповноваженого адміністратора вашої компанії — він запустить контрольоване відновлення."><div className="access-card-actions"><Button onClick={() => navigate('/login')}>До входу</Button></div></AccessCard>
...
function AccessCard({ icon, title, text, children, variant = 'default' }: { icon: React.ReactNode; title: string; text: string; children: React.ReactNode; variant?: 'default' | 'help' }) {
  ...
  return <main className="access-page"><section className={`access-card ${help ? 'access-card--help' : ''}`}><div className="access-card-header"><span className={help ? 'access-brand-lockup' : 'auth-brand'}><BrandMark size="lg" />{help ? <span className="access-brand-name"><strong>BERT</strong><span>CRM</span></span> : <strong>BERT CRM</strong>}</span><div className="access-icon">{icon}</div></div><div><h1>{title}</h1><p>{text}</p></div>{children}</section></main>
}
