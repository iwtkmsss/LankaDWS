...
export function LoginPage() {
  ...
  return <main className="auth-page">
    <section className="auth-panel"><div className="auth-brand"><BrandMark /><strong>LankaDWS</strong></div><div className="auth-copy"><span className="eyebrow">Внутрішній простір команди</span><h2>Робота, рішення й знання — в одному місці.</h2></div><img src="/assets/auth/editorial-workspace.png" alt="Світлий сучасний робочий простір LankaDWS" width="720" height="480" /></section>
    <section className="login-card"><div><span className="eyebrow">З поверненням</span><h1>Увійти до LankaDWS</h1></div>
      ...
      {__LANKADWS_DEMO_MODE__ && <aside className="demo-note"><strong>Демо-режим</strong><span>maria / andrii / olena / dmytro</span><code>{__LANKADWS_DEMO_PASSWORD__}</code></aside>}
    </section>
  </main>
}
...
export function RestrictedAccessPage({ mode }: { mode: 'first-login' | 'password-changed' | 'help' | 'setup' | 'challenge' }) {
  ...
  if (mode === 'help') return <AccessCard variant="help" icon={<KeyRound />} title="Допомога з доступом" text="У LankaDWS немає відновлення через email. Зверніться до уповноваженого адміністратора організації — він запустить контрольоване відновлення."><div className="access-card-actions"><Button onClick={() => navigate('/login')}>До входу</Button></div></AccessCard>
...
function AccessCard({ icon, title, text, children, variant = 'default' }: { icon: React.ReactNode; title: string; text: string; children: React.ReactNode; variant?: 'default' | 'help' }) {
  ...
  return <main className="access-page"><section className={`access-card ${help ? 'access-card--help' : ''}`}><div className="access-card-header"><span className={help ? 'access-brand-lockup' : 'auth-brand'}><BrandMark size="lg" />{help ? <span className="access-brand-name"><strong>LankaDWS</strong></span> : <strong>LankaDWS</strong>}</span><div className="access-icon">{icon}</div></div><div><h1>{title}</h1><p>{text}</p></div>{children}</section></main>
}
