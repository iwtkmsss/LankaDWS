...
export function LoginPage() {
  ...
  return <main className="auth-page">
    <section className="auth-panel"><div className="auth-brand"><span className="brand-mark">B</span><strong>BERT CRM</strong></div><div className="auth-copy"><span className="eyebrow">Внутрішній простір команди</span><h1>Робота, рішення й знання — в одному місці.</h1><p>Безпечний доступ до задач, заявок, документів і робочих процесів вашої компанії.</p></div><img src="/assets/auth/editorial-workspace.png" alt="Світлий сучасний робочий простір BERT" width="720" height="480" /></section>
    <section className="login-card"><div><span className="eyebrow">З поверненням</span><h2>Увійти до BERT CRM</h2><p>Використовуйте корпоративний нікнейм без символу @.</p></div>
      <form onSubmit={submit}><label>Нікнейм<div className="field-with-icon"><span>@</span><input autoComplete="username" value={username} onChange={(event) => setUsername(event.target.value.toLowerCase())} required pattern="[a-z0-9._-]{3,32}" placeholder="maria" /></div></label><label>Пароль<div className="field-with-icon"><LockKeyhole size={17} /><input autoComplete="current-password" type={show ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} required /><button type="button" aria-label={show ? 'Сховати пароль' : 'Показати пароль'} onClick={() => setShow((value) => !value)}>{show ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></label>{error && <div className="form-error" role="alert">{error}</div>}<Button disabled={busy}>{busy ? 'Перевіряємо…' : <>Увійти <ArrowRight size={17} /></>}</Button><a href="/access-help">Не вдається увійти?</a></form>
      {import.meta.env.VITE_DEMO_MODE === 'true' && <aside className="demo-note"><strong>Демо-режим</strong><span>maria / andrii / olena / dmytro</span><code>BertDemoPassphrase2026!</code></aside>}
    </section>
  </main>
}
...
export function RestrictedAccessPage({ mode }: { mode: 'first-login' | 'password-changed' | 'help' | 'setup' | 'challenge' }) {
  ...
  if (mode === 'help') return <AccessCard icon={<KeyRound />} title="Допомога з доступом" text="У BERT CRM немає відновлення через email. Зверніться до уповноваженого адміністратора вашої компанії — він запустить контрольоване відновлення."><Button onClick={() => navigate('/login')}>До входу</Button></AccessCard>
  ...
  return <AccessCard icon={<KeyRound />} title="Створіть власний пароль" text="Тимчасовий пароль більше не діятиме після збереження."><form onSubmit={change} className="compact-form"><label>Новий пароль<input type="password" minLength={15} value={password} onChange={(event) => setPassword(event.target.value)} required /></label><label>Повторіть пароль<input type="password" minLength={15} value={confirmation} onChange={(event) => setConfirmation(event.target.value)} required /></label>{error && <div className="form-error">{error}</div>}<Button>Зберегти пароль</Button></form></AccessCard>
}

function AccessCard({ icon, title, text, children }: { icon: React.ReactNode; title: string; text: string; children: React.ReactNode }) { return <main className="access-page"><section className="access-card"><div className="access-icon">{icon}</div><div><span className="auth-brand"><span className="brand-mark">B</span><strong>BERT CRM</strong></span><h1>{title}</h1><p>{text}</p></div>{children}</section></main> }
