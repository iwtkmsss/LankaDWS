...
function CompanyEditor({ onClose, company }: { onClose: () => void; company?: CompanyDetail }) {
          ...
          <section className="company-tree-section">
            ...
            <div className="company-tree" role="tree" aria-label={`Структура ${company.name}`}>
              ...
              {expanded.has('company') && <div role="group">
                {tree.length ? tree.map((unit) => <CompanyTreeUnit key={unit.id} unit={unit} depth={1} expanded={expanded} employees={people} onToggle={toggle} companyId={company.id} />) : <p className="company-tree__empty">Підрозділів ще немає.</p>}
                {unassigned.length > 0 && <div className="company-tree__unassigned"><span>Без підрозділу</span>{unassigned.map((employee) => <EmployeeTreeLink key={employee.id} employee={employee} companyId={company.id} depth={2} />)}</div>}
              </div>}
            </div>
          </section>
...
function CompanyTreeUnit({ unit, depth, expanded, employees, onToggle, companyId }: { unit: TreeUnit; depth: number; expanded: Set<string>; employees: Employee[]; onToggle: (id: string) => void; companyId: string }) {
  ...
  return (
    <div role="treeitem" aria-expanded={hasChildren ? isOpen : undefined}>
      ...
      {isOpen && <div role="group">{unit.children.map((child) => <CompanyTreeUnit key={child.id} unit={child} depth={depth + 1} expanded={expanded} employees={employees} onToggle={onToggle} companyId={companyId} />)}{members.map((employee) => <EmployeeTreeLink key={employee.id} employee={employee} companyId={companyId} depth={depth + 1} />)}</div>}
    </div>
  )
}

function EmployeeTreeLink({ employee, companyId, depth }: { employee: Employee; companyId: string; depth: number }) {
  return (
    <Link className="company-tree__person" style={{ '--tree-depth': depth } as CSSProperties} to={`/organization?companyId=${companyId}&view=people&employeeId=${employee.id}`}>
