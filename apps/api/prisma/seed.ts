...
const password = process.env.DEMO_SEED_PASSWORD ?? 'LankaDWSDemoPassphrase2026!'

const users = [
  { id: 'usr_maria', username: 'maria', displayName: 'Марія Іваненко', jobTitle: 'Продуктова дизайнерка', gender: 'FEMALE', birthDate: new Date('1994-05-12T00:00:00.000Z'), accountType: 'USER' as const, companyId: 'cmp_lankadws_ua', avatar: '/assets/avatars/avatar-maria.webp', approverId: 'usr_andrii' },
  { id: 'usr_andrii', username: 'andrii', displayName: 'Андрій Коваль', jobTitle: 'Операційний керівник', gender: 'MALE', birthDate: new Date('1989-11-03T00:00:00.000Z'), accountType: 'USER' as const, companyId: 'cmp_lankadws_ua', avatar: '/assets/avatars/avatar-andrii.webp', approverId: 'usr_dmytro' },
  { id: 'usr_olena', username: 'olena', displayName: 'Олена Бондар', jobTitle: 'HR-фахівчиня', gender: 'FEMALE', birthDate: new Date('1991-02-18T00:00:00.000Z'), accountType: 'USER' as const, companyId: 'cmp_lankadws_ua', avatar: '/assets/avatars/avatar-olena.webp', approverId: 'usr_andrii' },
  ...
  { id: 'usr_marko', username: 'marko', displayName: 'Марко Литвин', jobTitle: 'Дизайнер', gender: 'MALE', birthDate: new Date('1996-06-30T00:00:00.000Z'), accountType: 'USER' as const, companyId: 'cmp_lankadws_ua', avatar: '/assets/avatars/avatar-marko.webp', approverId: 'usr_andrii' },
]

async function seed(): Promise<void> {
  await prisma.workspace.upsert({ where: { id: 'ws_lankadws' }, create: { id: 'ws_lankadws', displayName: 'LankaDWS Workspace' }, update: { displayName: 'LankaDWS Workspace' } })
  await prisma.company.upsert({ where: { id: 'cmp_lankadws_ua' }, create: { id: 'cmp_lankadws_ua', workspaceId: 'ws_lankadws', displayName: 'LankaDWS', legalName: 'LankaDWS', code: 'lankadws', description: 'Продуктова та операційна команда спільного робочого простору LankaDWS.' }, update: { displayName: 'LankaDWS', legalName: 'LankaDWS', code: 'lankadws', description: 'Продуктова та операційна команда спільного робочого простору LankaDWS.' } })
  ...
  await prisma.company.update({ where: { id: 'cmp_lankadws_ua' }, data: { managerId: 'usr_andrii' } })
  ...
  if (!await prisma.feedPost.findUnique({ where: { id: 'feed_release_notes' } })) {
    ...
    await prisma.feedItem.create({
      data: {
        ...
        actorId: 'usr_andrii',
        visibility: 'COMPANY',
        safePayload: JSON.stringify({ postId: 'feed_release_notes', action: 'PUBLISHED', snippet: 'Оновили робочий простір LankaDWS' }),
  ...
  const orgUnits = [
    { id: 'org_lankadws_ua_operations', companyId: 'cmp_lankadws_ua', sourceKey: 'operations', parentId: null, name: 'Операції', normalizedName: 'операції', managerId: 'usr_andrii', sortOrder: 10 },
    { id: 'org_lankadws_ua_product', companyId: 'cmp_lankadws_ua', sourceKey: 'product', parentId: 'org_lankadws_ua_operations', name: 'Продукт і дизайн', normalizedName: 'продукт і дизайн', managerId: 'usr_maria', sortOrder: 20 },
    { id: 'org_lankadws_ua_people', companyId: 'cmp_lankadws_ua', sourceKey: 'people', parentId: 'org_lankadws_ua_operations', name: 'Люди та культура', normalizedName: 'люди та культура', managerId: 'usr_olena', sortOrder: 30 },
    { id: 'org_lankadws_ua_technology', companyId: 'cmp_lankadws_ua', sourceKey: 'technology', parentId: 'org_lankadws_ua_operations', name: 'Технології', normalizedName: 'технології', managerId: 'usr_andrii', sortOrder: 40 },
    { id: 'org_lankadws_service', companyId: 'cmp_lankadws_ua', sourceKey: 'service', parentId: 'org_lankadws_ua_operations', name: 'Сервісний відділ', normalizedName: 'сервісний відділ', managerId: 'usr_marko', sortOrder: 50 },
  ] as const
  ...
  for (const assignment of [
    { id: 'orga_andrii_operations', userId: 'usr_andrii', companyId: 'cmp_lankadws_ua', orgUnitId: 'org_lankadws_ua_operations', positionTitle: 'Операційний керівник' },
    { id: 'orga_maria_product', userId: 'usr_maria', companyId: 'cmp_lankadws_ua', orgUnitId: 'org_lankadws_ua_product', positionTitle: 'Продуктова дизайнерка' },
    { id: 'orga_olena_people', userId: 'usr_olena', companyId: 'cmp_lankadws_ua', orgUnitId: 'org_lankadws_ua_people', positionTitle: 'HR-фахівчиня' },
    { id: 'orga_marko_service', userId: 'usr_marko', companyId: 'cmp_lankadws_ua', orgUnitId: 'org_lankadws_service', positionTitle: 'Дизайнер' },
  ]) {
  ...
  await prisma.project.upsert({
    ...
    create: { id: 'prj_website', workspaceId: 'ws_lankadws', companyId: 'cmp_lankadws_ua', name: 'Вебсайт для клієнта', normalizedName: 'вебсайт для клієнта' },
  ...
  for (const tag of [
    ...
    await prisma.tag.upsert({
      where: { id: tag.id },
      create: { ...tag, workspaceId: 'ws_lankadws', companyId: 'cmp_lankadws_ua' },
  ...
  for (const [taskId, number, title, createdById, responsibleId, status, priority, dueAt, projectId] of taskData) {
    await prisma.task.upsert({
      where: { id: taskId },
      create: { id: taskId, workspaceId: 'ws_lankadws', companyId: 'cmp_lankadws_ua', number, title, createdById, reporterId: createdById, status, priority, dueAt: new Date(dueAt), projectId },
  ...
  for (const [eventId, ownerId, title, start, end] of [
    ...
    await prisma.event.upsert({ where: { id: eventId }, create: { id: eventId, workspaceId: 'ws_lankadws', companyId: 'cmp_lankadws_ua', ownerId, title, startAt: new Date(start), endAt: new Date(end), sourceTimezone: 'Europe/Kyiv' }, update: {} })
    await prisma.eventAudience.upsert({
      where: { eventId_companyId: { eventId, companyId: 'cmp_lankadws_ua' } },
      create: { id: `evta_${eventId}_lankadws_ua`, eventId, companyId: 'cmp_lankadws_ua' },
      update: {},
    })
  }

  await prisma.document.upsert({ where: { id: 'doc_policy' }, create: { id: 'doc_policy', workspaceId: 'ws_lankadws', companyId: 'cmp_lankadws_ua', number: 'DOC-0102', name: 'Політика роботи з даними', ownerId: 'usr_olena', status: 'PUBLISHED', confidentiality: 'INTERNAL' }, update: {} })
  await prisma.knowledgeArticle.upsert({ where: { id: 'art_security' }, create: { id: 'art_security', workspaceId: 'ws_lankadws', slug: 'bezpechna-robota-z-danymy', ownerId: 'usr_olena', currentVersionId: 'artv_security', reviewAt: new Date('2026-12-01T00:00:00Z') }, update: {} })
  ...
  for (const companyId of ['cmp_lankadws_ua', 'cmp_lankadws_service']) await prisma.articleAudience.upsert({ where: { articleId_principalType_principalId: { articleId: 'art_security', principalType: 'COMPANY', principalId: companyId } }, create: { id: `arta_${companyId}`, articleId: 'art_security', principalType: 'COMPANY', principalId: companyId }, update: {} })

  await prisma.announcement.upsert({ where: { id: 'ann_policy' }, create: { id: 'ann_policy', workspaceId: 'ws_lankadws', authorId: 'usr_olena', title: 'Оновлено політику роботи з даними', body: 'Перегляньте короткий перелік змін і підтвердьте ознайомлення у базі знань.', status: 'PUBLISHED', isPinned: true, publishAt: new Date('2026-07-12T09:00:00Z') }, update: {} })
  await prisma.announcementAudienceCompany.upsert({ where: { announcementId_companyId: { announcementId: 'ann_policy', companyId: 'cmp_lankadws_ua' } }, create: { id: 'anc_policy', announcementId: 'ann_policy', companyId: 'cmp_lankadws_ua' }, update: {} })
  ...
  const feedSourceItems = [
    ...
    },
    {
      ...
      sourceVersion: 1,
      action: 'PUBLISHED',
      eventKey: 'feed:announcement:ann_policy:cmp_lankadws_ua:v1:published',
      actorId: 'usr_olena',
      visibility: 'PARTICIPANTS',
  ...
  for (const source of feedSourceItems) {
    ...
    const allowedRecipients = await prisma.user.findMany({
      where: { primaryCompanyId: 'cmp_lankadws_ua', isActive: true, id: { in: [...source.recipientIds] } },
      select: { id: true },
    })
  ...
  await prisma.lifecycleProcess.upsert({ where: { id: 'life_onboarding' }, create: { id: 'life_onboarding', workspaceId: 'ws_lankadws', companyId: 'cmp_lankadws_ua', employeeId: 'usr_marko', processType: 'ONBOARDING', templateVersion: 1, ownerId: 'usr_olena', startAt: new Date('2026-07-20T00:00:00Z'), status: 'IN_PROGRESS', progress: 42 }, update: {} })
  ...
  await prisma.messageThread.upsert({ where: { id: 'thread_design' }, create: { id: 'thread_design', workspaceId: 'ws_lankadws', companyId: 'cmp_lankadws_ua', kind: 'CONTEXTUAL', title: 'Дизайн dashboard', entityType: 'TASK', entityId: 'tsk_design', createdById: 'usr_maria', lastMessageAt: new Date() }, update: {} })
  ...
  await prisma.auditEvent.create({ data: { id: `aud_seed_${Date.now()}`, workspaceId: 'ws_lankadws', actorType: 'SYSTEM', action: 'demo.seeded', entityType: 'WORKSPACE', entityId: 'ws_lankadws', result: 'SUCCESS', risk: 'NORMAL', correlationId: `seed_${Date.now()}` } })
}
