...
const sourceSpanMs =
  ...
  Date.parse('2018-12-01T00:00:00.000Z');
...
function sourceTime(
  ...
  return new Date(
    baseTime - Math.floor((position / denominator) * sourceSpanMs),
  ).toISOString();
}
...
function seedDatabase(
  ...
  const seed = database.transaction(() => {
    ...
    for (let index = 0; index < profile.posts; index += 1) {
      ...
      const currentVersion =
        index < profile.supersededPostSources ? profile.postVersions : 1;
      const liveDelta = index < Math.min(profile.posts, 200);
      const requiresAcknowledgement =
        !hiddenAudience &&
        acknowledgementOrdinal < Math.min(140, profile.posts);
      const acknowledgementVersion = requiresAcknowledgement
        ? currentVersion
        : 0;
    ...
    for (let index = 0; index < profile.comments; index += 1) {
      ...
      const createdAt = olderVersionTime(
        sourceTime(profile, postIndex, 0),
        index + 10,
      );
      insertComment.run(
        `cmt_feed_${pad(index)}`,
    ...
    for (let index = 0; index < profile.tasks; index += 1) {
      ...
      const creatorId =
        visibilityPattern === 2
          ...
          : userId((index % (profile.users - 1)) + 1);
      const assigneeId =
        visibilityPattern === 1 || visibilityPattern === 3
          ? principalId
  ...
  const totalHeads =
    profile.posts + profile.tasks + profile.events + profile.fileShares;
  return {
...
function assertPageOrder(result: FeedListResult): void {
  for (let index = 1; index < result.items.length; index += 1) {
    ...
    const previousTime =
      previous.kind === 'POST' ? previous.publishedAt : previous.occurredAt;
    const currentTime =
      current.kind === 'POST' ? current.publishedAt : current.occurredAt;
    assertCondition(
...
async function main(): Promise<void> {
  ...
  const iterations = parseIntegerFlag(
    '--iterations',
    profileName === 'representative' ? 20 : 4,
    1,
    100,
  );
  const warmup = parseIntegerFlag(
    '--warmup',
    profileName === 'representative' ? 3 : 1,
    0,
    20,
  );
  const outputValue =
    parseStringFlag('--output') ??
  ...
  console.log(
    `Fixture: ${profile.posts} posts, ${profile.tasks} tasks, ${profile.events} events, ` +
      `${profile.fileShares} synthetic file shares`,
  );
  ...
  try {
    ...
    assertCondition(
      integrity.mismatchedHeads === 0,
      'all heads must match their immutable item',
    );
    const expectedIndex =
      'FeedSourceHead_workspaceId_companyId_occurredAt_itemId_idx';
    const expectedUnreadIndex =
      'FeedSourceHead_workspaceId_companyId_countsAsUnread_occurredAt_itemId_idx';
    assertCondition(
    ...
    assertCondition(
      ...
      'representative first page must have a cursor',
    );
    assertCondition(
    ...
    assertCondition(
      firstPage.attention.pendingAcknowledgements ===
        seedEvidence.expectedPendingAcknowledgements,
    ...
    for (let pageIndex = 0; pageIndex < 20; pageIndex += 1) {
      ...
      for (const entry of page.items) {
        ...
        assertCondition(
          !traversedKeys.has(key),
          `cursor traversal repeated ${key}`,
        );
    ...
    for (const type of ['POST', 'TASK', 'EVENT', 'FILE'] as const) {
      const result = await feed.list(principal, { ...baseQuery, type });
      typeResults.set(type, result);
      assertCondition(
        result.items.length > 0,
        `${type} scenario must return visible rows`,
      );
      assertCondition(
    ...
    assertCondition(
      acknowledgementResult.items.length > 0 &&
        acknowledgementResult.items.every(
          (entry) =>
    ...
    const evidence = {
      ...
      caveat:
        'This is a deterministic local baseline, not production capacity approval and not a substitute for DDB-004 or D-020.',
      sourceBasis: {
        ...
        syntheticExtension: {
          ...
          reason:
            'Target-native file share cards have no approved Bitrix source count yet; the bounded fixture exercises the new ACL/scanner query branch without claiming source representativeness.',
        },
        liveDeltaAssumption: {
          heads: seedEvidence.liveDeltaHeads,
          reason:
            'Historical materialization is silent and seeds read cursors; only a bounded synthetic post-cutover delta counts as unread.',
        },
      },
      isolation: {
      ...
      correctness: {
        ...
        checks: [
          ...
          'post/task/event/file ACL canaries remain hidden',
      ...
      queryPlans: {
        ...
        temporarySortObserved: [...corePlan, ...unreadPlan].some((row) =>
...
main().catch((error: unknown) => {
  console.error(
    error instanceof Error ? error.message : 'Feed rehearsal failed',
  );
  process.exitCode = 1;
});
