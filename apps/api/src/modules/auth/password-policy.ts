...
export function assertPasswordPolicy(password: string, username: string, twoFactorRequired = false): void {
  ...
  if (commonValues.has(normalized) || normalized.includes('bert') || normalized.includes(username.toLowerCase())) {
    throw badRequest('password_blocklisted', 'Оберіть довшу парольну фразу, не пов’язану з BERT CRM або нікнеймом.')
  }
}
