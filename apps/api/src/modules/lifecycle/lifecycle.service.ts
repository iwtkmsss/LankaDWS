...
interface StartInput { companyId?: string; employeeId: string; startAt: string; endAt?: string }
...
export class LifecycleService {
  ...
  async list(principal: AuthPrincipal, company?: string) {
    const rows = await this.prisma.lifecycleProcess.findMany({ where: { companyId: { in: this.scope.allowedCompanies(principal, company) }, processType: 'OFFBOARDING' }, include: { steps: true }, orderBy: { updatedAt: 'desc' } })
    return { items: rows }
  }

  async detail(principal: AuthPrincipal, processId: string) {
    const process = await this.prisma.lifecycleProcess.findFirst({ where: { id: processId, companyId: { in: principal.allowedCompanyIds }, processType: 'OFFBOARDING' }, include: { steps: { orderBy: { dueAt: 'asc' } } } })
  ...
  async start(principal: AuthPrincipal, input: StartInput) {
    ...
    await this.taskNumbers.runInTransaction(this.prisma, async (tx, firstNumber) => {
      await tx.lifecycleProcess.create({ data: { id: processId, workspaceId: principal.workspaceId, companyId, employeeId: employee.id, processType: 'OFFBOARDING', templateVersion: 1, ownerId: principal.userId, startAt, endAt: input.endAt ? new Date(input.endAt) : null, status: 'IN_PROGRESS' } })
      ...
      await tx.auditEvent.create({ data: { id: id('aud'), workspaceId: principal.workspaceId, companyId, actorType: 'USER', actorId: principal.userId, action: 'lifecycle.offboarding.started', entityType: 'LIFECYCLE', entityId: processId, result: 'SUCCESS', risk: 'HIGH', correlationId: id('corr') } })
    })
  ...
  async complete(principal: AuthPrincipal, processId: string, expectedVersion: number, ownershipTransferred: boolean) {
    const process = await this.prisma.lifecycleProcess.findFirst({ where: { id: processId, companyId: { in: principal.allowedCompanyIds }, processType: 'OFFBOARDING' }, include: { steps: true } })
    if (!process) throw notFound()
    if (process.version !== expectedVersion) throw conflict()
    const incomplete = process.steps.filter((step) => step.status !== 'DONE')
    if (incomplete.length || !ownershipTransferred) throw badRequest('lifecycle_blocked', 'Критичні кроки або передача власності ще не завершені.')
