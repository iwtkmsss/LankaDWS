...
export class SavedViewsController {
  ...
  @Get()
  list(@Req() request: LankaDWSRequest) { return this.prisma.savedView.findMany({ where: { userId: principalFrom(request).userId }, orderBy: [{ module: 'asc' }, { name: 'asc' }] }) }

  @Post()
  save(@Req() request: LankaDWSRequest, @Body() body: { module: string; name: string; queryState: Record<string, unknown>; isDefault?: boolean }) {
    const userId = principalFrom(request).userId
    const module = body.module?.trim().toUpperCase()
    const name = body.name?.trim()
    if (!module || !['TASKS', 'DOCUMENTS', 'EMPLOYEES', 'FEED'].includes(module) || !name || name.length > 80 || !body.queryState || typeof body.queryState !== 'object' || Array.isArray(body.queryState)) throw badRequest('saved_view_fields')
  ...
  @Delete(':id')
  async remove(@Req() request: LankaDWSRequest, @Param('id') viewId: string) {
