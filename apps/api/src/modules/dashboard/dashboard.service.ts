...
export class DashboardService {
  ...
  async get(principal: AuthPrincipal): Promise<DashboardView> {
    const [tasks, eventRows, receipts, lifecycle] = await Promise.all([
    ...
    return {
      attentionCount: tasks.items.filter((task) => task.status === 'BLOCKED' || (task.deadline && new Date(task.deadline) < new Date())).length,
      tasks: tasks.items,
