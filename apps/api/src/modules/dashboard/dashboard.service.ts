...
export class DashboardService {
  ...
  async get(principal: AuthPrincipal): Promise<DashboardView> {
    ...
    const companyId =
      principal.primaryCompanyId ?? principal.allowedCompanyIds[0];
    ...
    const completedSince = this.zonedDateBoundary(
      this.shiftDate(localDate, -6),
      timezone,
      false,
    );
    ...
    const activity =
      feedData?.items.map((item) => this.activityItem(item)) ?? [];
    const attentionCount =
      (taskSummary?.attention ?? 0) +
  ...
  private async lifecycle(
    ...
    const employeeById = new Map(
      employees.map((user) => [user.id, user.displayName]),
    );
  ...
  private nextStep(input: {
    ...
    if (task) {
      return {
        kind: 'TASK',
        title: task.title,
        detail: `${task.number} · прострочено`,
        href: `/tasks/${task.id}`,
      };
    }
    ...
    const event = input.eventData?.items.find(
      (item) => new Date(item.startAt) <= input.todayEnd,
    );
    if (event) {
      return {
        kind: 'EVENT',
    ...
    if (lifecycle) {
      return {
        kind: 'LIFECYCLE',
        title: `Офбординг · ${lifecycle.employeeName}`,
        detail: `${lifecycle.progress}% процесу завершено`,
        href: `/offboarding/${lifecycle.id}`,
      };
    }
    return {
      kind: 'KNOWLEDGE',
      title: 'Оновлено політику роботи з даними',
      detail: 'Перегляньте короткий перелік змін у базі знань.',
      href: '/knowledge',
    };
  }
  ...
  private actionLabel(kind: DashboardNextStep['kind']): string {
    ...
    if (kind === 'KNOWLEDGE') return 'Відкрити базу знань';
    return 'Відкрити';
  }
