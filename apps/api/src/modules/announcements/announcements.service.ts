...
export class AnnouncementsService {
  ...
  async list(principal: AuthPrincipal, company?: string, state = 'active') {
    ...
    return { items: receipts.map((receipt) => ({ id: receipt.announcement.id, title: receipt.announcement.title, safeSnippet: receipt.announcement.body.slice(0, 180), authorName: authorById.get(receipt.announcement.authorId) ?? 'LankaDWS', companyIds: receipt.announcement.companies.map((entry) => entry.companyId), status: receipt.announcement.status, isPinned: receipt.announcement.isPinned, publishedAt: receipt.announcement.publishAt?.toISOString() ?? null, readAt: receipt.readAt?.toISOString() ?? null })) }
  }
