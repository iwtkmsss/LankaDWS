...
export async function createAdmin(prisma: PrismaService): Promise<void> {
  ...
  const companyName = await promptText('Назва організації')
  const legalName = await promptText('Юридична назва організації', companyName)
  const companyCode = (await promptText('Системний код організації')).toLowerCase()
  ...
  stdout.write(`Створено першого адміністратора @${username}. Organization ID: ${companyId}. Наступний крок: увійдіть і налаштуйте 2FA.\n`)
}
