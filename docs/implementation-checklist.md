...
- [x] Offboarding with linked tasks and ownership-transfer blockers.
...
- [x] Feed attachments reuse `FileObject`/`FileLink`, quarantine/scanner and source ACL; Task block/assignment, Event and File render as compact canonical cards whose immutable projection stores no editable source body.
...
- [x] One permission/capability-aware Quick Create is available from desktop/mobile topbar and as the first `Ctrl/Cmd+K` group for Task, Chat, Calendar Event, Request, Group and Drive upload. It reuses canonical forms, persists company scope and resolves `company=all` to a real enabled company without a migration.
