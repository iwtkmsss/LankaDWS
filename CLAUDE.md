...
- Default validation is focused per workspace: `npm run typecheck|lint|test --workspace @bert-crm/<contracts|api|web>`. `npm run quality` and `npm run test:e2e` are release gates, not local defaults.
